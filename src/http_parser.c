        ...
        if (request->header_count < MAX_HEADERS) {
            if (parse_header_line(header_line, &request->headers[request->header_count]) == 0) {
                ...
                // 缓存Connection头部
                    ...
                    strncpy(request->connection, header->value, MAX_HEADER_VALUE - 1);
                    if (strcmp(header->value, "keep-alive") == 0) {
                        request->content_length = 1;
                    }
                }
                // 读取Content-Length，确定请求体长度
                else if (strcmp(header->name, "Content-Length") == 0) {
...
void http_request_print(const http_request_t *request) {
    ...
    log_info("Connection: %s", request->connection);
